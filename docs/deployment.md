...
Use [XML encoding and file handoff](xml-encoding-and-file-handshake.md) as the commissioning reference for BOM handling, writer-close detection, retries, and the recommended equipment-side atomic publication sequence.
...
- Deploy the build containing all 25 Action templates for Stage/Camera/Focus/Integration/Live/Abort/OM/Lens/ACB: nine request files, nine success-response files, and seven common-only failure-response files. They must match the actual equipment schema. Embedded templates and inbound wire XML accept strict UTF-8 with zero or one leading BOM; that BOM is removed in memory. Invalid UTF-8, UTF-16/32, double/embedded U+FEFF and NUL-bearing text are rejected. App-generated request/test-response wire files are always UTF-8 without BOM, and all wire payloads are limited to 4 MiB. Strings are XML-escaped, and metre values are emitted as invariant uppercase scientific notation with at least two exponent digits (`1E-06`). Response parsing accepts padded or unpadded exponents and ignores ASCII space/tab/CR/LF differences throughout fixed declaration/tag/indentation text while preserving whitespace inside extracted logical values. Equivalent well-formed pretty-printed and single-line responses therefore both match; malformed XML is rejected. The JSON-like message shown in diagnostics and test dialogs is only an in-memory logical representation. An oversized response is ignored until the configured response timeout.
- Require the equipment to close its response writer before the file becomes consumable. Prefer writing and flushing a unique temporary file in the exchange directory, closing it, and atomically moving/replacing it at the configured response filename. Validate this behavior on the real local/SMB target; keeping the final file open for writing causes the app to retry until timeout when the SMB server honors Windows share modes.
...
- all nine Designer equipment Actions publish XML from the correct template and accept only a response whose `type`, `correlation_id`, and `action` match; successful responses require every Action-specific field, while `result = 1` may contain only the common fields, preserves that failure result, and faults the remaining workflow;
...
- template and equipment fixtures accept UTF-8 both without BOM and with exactly one leading BOM, produce identical logical messages after normalization, emit canonical BOM-free app output, and reject invalid UTF-8, UTF-16/32, double/embedded U+FEFF, and NUL-bearing payloads;
- Stage/Camera finite signed coordinates have no artificial ±distance limit; Focus enforces strict HFW/range/step constraints; Integration enforces power-of-two frame counts through 64 and absolute image paths; Live fixes frame count to 1; OM requires an absolute image filename; Lens accepts only `lens1`/`lens2`/`no_change` and returns only actual `lens1`/`lens2`; ACB enforces `0 < hfw < 2.4E-3 m`; Abort has no request/response fields beyond its exact common envelope;
...
- high-quality Integration, Lens, and ACB likewise preempt the active Live request, never overlap request files, and restore only the prior streaming intent; Lens success keeps image-based movement unavailable until a newly decoded Live frame, and Live exposes neither OM nor Abort;
- every Live Interaction right-panel card—including state, latest target, controls, and capture results—starts collapsed, can be expanded independently, and remains reachable through the panel's own vertical scrollbar at small window sizes and high DPI;
...
- a response written while its writer handle remains open: the exchange must stay pending when the local filesystem or SMB server honors Windows share modes, then complete after the writer closes; also verify the equipment's same-directory temp/flush/close/atomic-rename path on local and SMB storage;
