...
Publishing uses a complete temporary file followed by a same-directory replace/move. The transport holds the fixed `.drillflow.exchange.lock` sidecar open with `FileShare.None` for the entire request/response exchange. Windows and SMB therefore serialize separate app processes or workstations that point at the same directory; lock acquisition is cancellable and bounded by the configured response timeout, and it never publishes a request on timeout. Response observation uses stable-file polling and retries transient sharing violations. The Action-specific XML response template must parse successfully, `type` must be `response`, `correlation_id` and `action` must match the in-flight request, `result` must be `0` or `1`, and every Action-specific field must satisfy its contract before the response is accepted.
