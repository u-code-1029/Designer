...
> 상태: 구현 기준 문서 · 계약 버전 6 · 최종 확인 2026-08-28
...
| `action` | string | `stage`, `camera`, `focus`, `integration`, `live`, `abort`, `om`, `lens`, `acb` 중 하나 |
...
숫자는 논리적으로 JSON number이며 XML에서는 invariant culture의 과학적 표기법으로 기록한다. 모든 metre 값은 `NaN`, `Infinity`, `-Infinity`가 아닌 유한한 `double`이어야 한다. 실패 response(`result = 1`)는 `type`, `correlation_id`, `action`, `result`만 있으면 되고 성공 전용 Action 필드는 생략할 수 있다. 장비가 실패 response에도 성공 shape의 필드를 넣으면 앱은 호환을 위해 읽되 그 값은 신뢰하거나 요구하지 않는다.

## 3. Action 계약

Designer의 장비 Action은 아래 아홉 개다. 예전 `move`는 `stage`로 schema migration하고, 기존 Expression의 `parameters.move_x/y`, `result/last.stage_x/y`, `results[n].stage_x/y`, `index`, `command` 참조도 각각 `stage_x/y`, `current_stage_x/y`, `correlation_id`, `type`으로 갱신한다. 문자열 literal 안의 텍스트는 바꾸지 않는다. `measure`와 `drill`은 더 이상 지원하지 않는다. Delay/Repeat/Conditional/HTTP는 Designer 내부 Action이며 장비 파일을 만들지 않는다.
...
Abort request는 `type`, `correlation_id`, `action`만, response는 여기에 `result`만 더한 **정확한 공통 shape**를 사용한다.
...
~~~json
{
  ...
  "correlation_id": 7,
  "action": "om",
  "image_path": "C:\\EquipmentImages\\om-7.bmp"
}
~~~
...
- `image_path`: 파일명을 포함한 절대 Windows 로컬 경로 또는 UNC 경로. 빈 값과 상대 경로는 금지한다.
- 장비는 request와 다른 최종 절대 경로를 반환할 수 있으며, 앱은 성공 response 경로를 최종 결과로 사용한다.
- OM은 Designer 장비 Action이며 Live Interaction 페이지에는 제공하지 않는다.

### 3.8 Lens change — `lens`
...
~~~json
{
  ...
  "action": "lens",
  "result": 0,
  "current_lens_mode": "lens1"
}
~~~

- request `lens_mode`: 정확히 `lens1`, `lens2`, `no_change` 중 하나다.
- 성공 response `current_lens_mode`: 실제 장착 상태이므로 정확히 `lens1` 또는 `lens2`만 허용하며 `no_change`는 허용하지 않는다.
- Live Interaction에서도 다른 비-Live 장비 동작과 같은 독점 exchange로 실행한다. 성공 뒤에는 새 Live frame을 받을 때까지 기존 영상의 위치 보정을 오래된 상태로 취급한다.

### 3.9 Auto Contrast/Brightness — `acb`
...
- `hfw`: metre 기준 유한 number이며 `0 < hfw < 2.4E-3`이어야 한다. 소수와 과학적 표기 입력을 모두 허용하고 request XML에는 정규화된 과학적 표기로 쓴다.
- response에는 성공/실패 모두 공통 필드 외의 추가 필드가 없다.
- Live Interaction에서는 현재 frame HFW를 사용해 실행하고, 완료 뒤 Live frame 요청을 재개한다.
...
=om_1.result.image_path
=lens_1.result.current_lens_mode
~~~

성공한 장비 result에는 `type`, `correlation_id`, `action`, `result`와 Action별 response field가 포함된다. Repeat 내부 결과는 iteration마다 `results`에 모두 보존한다. response `result = 1`은 성공 전용 필드가 없어도 공통 실패 결과를 해당 Action에 먼저 보존한 뒤 Workflow를 Fault로 종료한다. 런타임 결과는 새 전체 Run, New/Open, 명시적 전체 초기화 또는 프로세스 종료 전까지 메모리에 남는다.

## 5. Live Interaction의 독점 동작

Live 페이지는 `live`, `integration`, `stage`, `camera`, `focus`, `lens`, `acb`를 사용할 수 있다. `om`과 `abort`는 제공하지 않는다. 오른쪽의 현재 상태, 최근 목표, Stage/Camera/HFW/Pixel Pitch/Focus/Lens/ACB 제어, 촬영 결과를 포함한 모든 카드 항목은 expandable card이며 접힌 상태로 시작한다.

- frame streaming 중 다른 장비 동작을 시작하면 active live request를 즉시 취소하고 자신이 게시한 request만 회수한다.
- Stage/Camera/Focus/Integration/Lens/ACB response가 올 때까지 새 live request를 만들지 않는다.
...
- Stage/Camera/Focus/Abort/Lens/ACB는 이미지 생성 UI를 표시하지 않는다.
- Integration/Live/OM은 768×512 모자이크 bitmap을 LocalAppData의 앱 전용 임시 폴더에 만들고 `image_path` 기본값으로 사용한다.
...
├─ Stage/request.xml, response.xml, failure-response.xml
├─ Camera/request.xml, response.xml, failure-response.xml
├─ Focus/request.xml, response.xml, failure-response.xml
├─ Integration/request.xml, response.xml, failure-response.xml
├─ Live/request.xml, response.xml, failure-response.xml
├─ Abort/request.xml, response.xml
├─ Om/request.xml, response.xml, failure-response.xml
├─ Lens/request.xml, response.xml, failure-response.xml
└─ Acb/request.xml, response.xml
~~~
...
9. 하나의 논리 값을 서로 다른 여러 field로 분해·결합해야 한다면 `XmlTemplateEquipmentMessageCodec`의 field adapter와 이 문서를 함께 바꾼다.
...
| 25개 XML 정답지 | `DrillFlow.Infrastructure/Communication/Templates/` |
